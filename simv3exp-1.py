"""SouDevail. Experiment 1. Initial Self-Healing mechanism test."""
import torch
...
class GumbelTopK(nn.Module):
    ...
    def forward(self, logits, k, training=True):
        if training:
            g = -torch.log(-torch.log(torch.rand_like(logits) + 1e-9) + 1e-9)
            scores = (logits + g) / self.tau
        else:
            scores = logits
...
class DFSHE(nn.Module):
    ...
    def forward(self, x):
        ...
        mask_m, w_m, idx_m = GumbelTopK(cfg.TAU)(logits_m, cfg.K_MACRO, self.training)
        ...
        _, w_micro, _ = GumbelTopK(cfg.TAU)(logits_micro, cfg.N_NANO, self.training)
        ...
        attn = q * ((k * v).sum(-1, keepdim=True)) / cfg.D_COMPACT
...
def run_training():
    ...
    print("epoch,loss,rollback_count,best_loss")
    ...
    for epoch in range(cfg.EPOCHS):
        ...
        if not controller.step(loss):
            print(f"{epoch},{loss.item():.6f},{controller.rollback_count},{controller.best_loss:.6f}")
        ...
        if (epoch + 1) % 10 == 0:
            print(f"{epoch+1},{loss.item():.6f},{controller.rollback_count},{controller.best_loss:.6f}")
            
        if loss.item() < cfg.TARGET_LOSS:
            print(f"{epoch+1},{loss.item():.6f},{controller.rollback_count},{controller.best_loss:.6f}")
    ...
    print(f"FINAL_STATUS,{loss.item() < cfg.TARGET_LOSS}")
