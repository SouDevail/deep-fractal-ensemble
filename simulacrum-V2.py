"""SouDevail. Simulacrum V2. Baseline fractal architecture with macro/micro routing."""
import torch
...
d_in, d_compact, d_common, d_out = 64, 4, 8, 10
...
class DFSHE(nn.Module):
    def __init__(self):
        ...
        self.mlp1_w = nn.Parameter(torch.randn(n_macro, d_compact, d_compact) * 0.1)
        self.mlp1_b = nn.Parameter(torch.zeros(n_macro, d_compact))
    ...
    def forward(self, x):
        ...
        mask_m, w_m, idx_m = gumbel_topk(logits_m, k_macro)
        ...
        m_w = self.micro_w[idx]
        m_b = self.micro_b[idx]
        logits_micro = torch.einsum('bd,bnd->bn', h_b, m_w) + m_b
        logits_micro = logits_micro.view(B, k_macro, n_nano)
        _, w_micro, _ = gumbel_topk(logits_micro, k_nano)
        #nanoexperts
        ...
        k = torch.einsum('bd,bcd->bc', h_b, self.attn_k[idx])
        v = torch.einsum('bd,bcd->bc', h_b, self.attn_v[idx])
        attn = q * ((k * v).sum(-1, keepdim=True)) / d_compact
        ...
        mlp1 = F.relu(torch.einsum('bd,bcd->bc', h_b, self.mlp1_w[idx]) + self.mlp1_b[idx])
        nano = torch.stack([mlp0, cnn, attn, tree, mlp1], dim=1).view(B, k_macro, n_nano, d_compact)
